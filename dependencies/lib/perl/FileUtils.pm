...
sub sort_delimited_file {
  my (%options) = @_;
  my $infile = $options{"-file"} || die "-file";
  my $outfile = $options{"-out"} || die "-out";
  my $field = $options{"-field"} || die "-field";
  my $delimiter = $options{"-delimiter"} || "\t";
  open(SDFTMP_IN, $infile) || die $!;
  my $header_line = <SDFTMP_IN>;
  chomp $header_line;
  ...
  my $sort_cmd = sprintf '|sort -t "%s" -k%d,%d >> %s', $delimiter, $field_number, $field_number, $outfile_tmp;
