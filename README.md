...
- `bamfile` is the input bamfile. Suggest mapping based on human GRCh37-lite or GRCh38_no_alt reference genome. Contact us if your bam is based on other reference version.
...
CICERO final fusion output file: $outdir/CICERO_DATADIR/*/final_fusions.txt. Use the following [guide](https://www.stjude.cloud/docs/guides/genomics-platform/analyzing-data/rapid-rnaseq/) to interpret the results.

Visualize CICERO fusion output: load the final fusion output file to https://proteinpaint.stjude.org/FusionEditor/
...
RNApeg is required to generate a junctions file (*.junctions.tab.shifted.tab) for use by CICERO. You can get RNApeg from both Docker and Singularity.
